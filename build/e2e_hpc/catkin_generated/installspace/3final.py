...
def distance_callback(data):
    global ekf, self_x, self_P, self_Q, self_R, self_X,AngleOfArrival_old, TimeOfFlight_0_old, TimeOfFlight_1_old, heading , theta_res , radii_res , value,label, counter, counter2, y, TimeOfFlight, AngleOfArrival, StrongestPathPower, old_val_count, old_val_theta,TimeOfFlight_0_mean,TimeOfFlight_1_mean, start,func
    #scan lay goc
    global x_data, y_data_1, y_data_2 , count, kf
    x = 0
    y = 0
    heading = -35
    ...
    alpha = 0.2
    AntennaDistance = 1
    #print(data.TimeOfFlight*100)
    #print("---")
    #print(data.AoA)
    # ekf.predict()
    # ekf.update(np.array([data.TimeOfFlight, 20]), anchor1)
    # print(ekf.current_position())
        
         
    TimeOfFlight_0 = float(data.TimeOfFlight) 
    AngleOfArrival = float(data.AoA)
    
    TimeOfFlight_0 = TimeOfFlight_0*alpha + TimeOfFlight_0_old*(1-alpha )
    TimeOfFlight_0_old = TimeOfFlight_0
    print("lowpass " + str(TimeOfFlight_0))
    print(rmse(TimeOfFlight_0,0.6))
    # AngleOfArrival = AngleOfArrival*alpha + AngleOfArrival_old*(1-alpha )
    # AngleOfArrival_old = AngleOfArrival

    
    TimeOfFlight_0 = kf.update(data.TimeOfFlight)
    print("kalman " + str(TimeOfFlight_0))
    print(rmse(TimeOfFlight_0,0.6))
    x_data.append(count)
    y_data_1.append(TimeOfFlight_0)
    y_data_2.append(data.TimeOfFlight)
    line_1.set_data(x_data, y_data_1)
    line_2.set_data(x_data, y_data_2)
    count+=1    
    

    scan_angle = AngleOfArrival * np.pi / 180
    rmin = TimeOfFlight_0        
    ...
    text = ''
    text += f'\n Distance: {float(rmin):.4} (m) \n  Angle: {float(scan_angle*180/np.pi):.4} (Deg)'
    print(text)
...
def listener():
    ...
    rospy.Subscriber("/chatter", Localize, distance_callback)
